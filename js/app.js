...
function cardHtml(c,color){
  ...
  return `<div class="card${c.id===sel?' sel':''}" id="c${c.id}"
    ...
    <div class="card-hd">
      ...
      <span class="card-lbl">${esc(cardLabel(c))}</span>
...
function buildHighlightRules() {
  // Gather enclosures: {open, close, multiLine, tokenType}
  const encs = doc.enclosedby.map(c=>({
    open:c.openSeq, close:c.closeSeq, multi:c.multiLine, type:c.tokenType
  }))
  // Gather keyword sets: Map<word_lower, tokenTypeName>
  const kwMap = new Map()
  doc.keygroup.forEach(g=>{
    g.words.forEach(w=>kwMap.set(w.toLowerCase(), g.groupName))
  })
  // Gather token type styles
  const styleMap = {}
  doc.tokentype.forEach(c=>{
    const style={}
    if(c.hasFG) style.color = colourToHex(c.fg)
    if(c.hasBG) style.background = colourToHex(c.bg)
    if(c.hasStyle){
      if(c.bold)      style.fontWeight='bold'
      if(c.italic)    style.fontStyle='italic'
      if(c.underline) style.textDecoration='underline'
      if(c.strikeout) style.textDecoration=(style.textDecoration?style.textDecoration+' ':'')+'line-through'
    }
    styleMap[c.name]=style
  })
  // Ident chars from header
  const hdr = doc.header[0]
  const identStartChars  = hdr ? charSetToRegexClass(hdr.identStart)  : '[A-Za-z_]'
  const identContChars   = hdr ? charSetToRegexClass(hdr.identCont)   : '[A-Za-z0-9_]'
  return {encs, kwMap, styleMap, identStartChars, identContChars}
}

// Parse a .msg char set expression like "'_', 'a'..'z', 'A'..'Z'"
// and return a regex character class string like "[_a-zA-Z]"
function charSetToRegexClass(expr) {
  ...
  while((m=re.exec(expr))!==null){
    // Add any literals before this range
    const before = expr.slice(last, m.index)
    const lits = [...before.matchAll(/'(.)'/g)]
    lits.forEach(l=>{ cls += escRegCls(l[1]) })
    cls += escRegCls(m[1])+'-'+escRegCls(m[2])
    last = m.index+m[0].length
  }
  // remaining literals
  const tail = expr.slice(last)
  const lits = [...tail.matchAll(/'(.)'/g)]
  lits.forEach(l=>{ cls += escRegCls(l[1]) })
...
function highlightSample(text) {
  if(!text) return ''
  const {encs, kwMap, styleMap, identStartChars, identContChars} = buildHighlightRules()
  const identRe = new RegExp(identStartChars + identContChars + '*', 'g')

  const lines = text.split('\n')
  let inEnc = null  // currently open enclosure or null
  let result = ''
  ...
  for(let li=0; li<lines.length; li++){
    ...
    // If we're inside a multi-line enclosure, continue it
    if(inEnc) {
      const closeIdx = line.indexOf(inEnc.close)
      if(closeIdx >= 0) {
        const chunk = line.slice(0, closeIdx+inEnc.close.length)
        const style = styleMap[inEnc.type]||{}
        lineOut += `<span${styleToAttr(style)}>${esc(chunk)}</span>`
        pos = closeIdx+inEnc.close.length
        inEnc = null
      } else {
        const style = styleMap[inEnc.type]||{}
        lineOut += `<span${styleToAttr(style)}>${esc(line)}</span>`
        pos = line.length
      }
    }
    ...
    while(pos < line.length) {
      ...
      // Try each enclosure opener
      for(const enc of encs) {
        if(line.startsWith(enc.open, pos)) {
          const closeIdx = line.indexOf(enc.close, pos+enc.open.length)
          const style = styleMap[enc.type]||{}
          if(closeIdx >= 0) {
            const chunk = line.slice(pos, closeIdx+enc.close.length)
            lineOut += `<span${styleToAttr(style)}>${esc(chunk)}</span>`
            pos = closeIdx+enc.close.length
          } else if(enc.multi) {
            // extends to end of line — continue on next
            const chunk = line.slice(pos)
            lineOut += `<span${styleToAttr(style)}>${esc(chunk)}</span>`
            pos = line.length
            inEnc = enc
          } else {
            // single-line — extends to EOL
            const chunk = line.slice(pos)
            lineOut += `<span${styleToAttr(style)}>${esc(chunk)}</span>`
            pos = line.length
          }
      ...
      if(idm && idm.index === pos) {
        ...
        const style = kwType ? (styleMap[kwType]||{}) : (styleMap['Identifier']||{})
        lineOut += `<span${styleToAttr(style)}>${esc(word)}</span>`
        pos += word.length
        continue
      }
      ...
      if(line[pos]===' '||line[pos]==='\t'){
        let wsEnd=pos
        while(wsEnd<line.length&&(line[wsEnd]===' '||line[wsEnd]==='\t')) wsEnd++
        const style = styleMap['Space']||{}
        lineOut += `<span${styleToAttr(style)}>${esc(line.slice(pos,wsEnd))}</span>`
        pos=wsEnd
        continue
      }

      // Single character fall-through
...
function buildSampleText(resultExpr) {
  // Replace all #NN (including #13#10 runs) with real chars, then extract strings
  let expr = resultExpr
  // Replace concatenated char consts between strings with newline markers
  expr = expr.replace(/\s*\+\s*(?:#\d+\s*)+\s*\+\s*/g, '\n')
  // Remaining leading/trailing char consts
  expr = expr.replace(/#13#10/gi, '\n').replace(/#10/gi, '\n').replace(/#13/gi, '\n')
  expr = expr.replace(/#(\d+)/g, (_, n) => String.fromCharCode(parseInt(n,10)))
  // Now extract all single-quoted Pascal strings
  const frags = []
  const re = /'((?:[^']|'')*)'/g
  let m
  while ((m = re.exec(expr)) !== null)
    frags.push(m[1].replace(/''/g, "'"))
  return frags.join('').replace(/\r\n/g, '\n').replace(/\r/g, '\n')
}
...
function parsePasFile(rawSrc, filename) {
  ...
  /* ── 8. GetSampleSource ─────────────────────────────────────────────────── */
  let sampleText = ''
  // Match the function including its return type on the same or next line
  const sampleFnRe = /function\s+\w+\.GetSampleSource\b[\s\S]*?\n\s*begin\b([\s\S]*?\n)\s*end\s*;/i
  const sampleM    = src.match(sampleFnRe)   // use original src (not stripped)
  if (sampleM) {
    const body  = sampleM[1]
    // Find Result := expression, which may span many lines with + continuation
    const resM  = body.match(/Result\s*:=([\s\S]*?);(?=[\s\n]*end\b)/i)
    if (resM) {
      sampleText = buildSampleText(resM[1])
    }
  }
