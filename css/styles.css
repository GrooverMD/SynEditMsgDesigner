...
*/
:root {
  --s0:#0f1117; --s1:#171b26; --s2:#1e2334; --s3:#252c3f; --s4:#2e3650; --s5:#3d4a6b;
  --t0:#e8ecf4; --t1:#8b92a8; --t2:#525b75;
  --a:#6c8aff; --a-bg:rgba(108,138,255,.12); --a-bd:rgba(108,138,255,.4);
  --d:#f06060;  --d-bg:rgba(240,96,96,.1);
  --lh:#9d8cff; --lt:#52c98a; --lk:#f4a44a; --lc:#f07070; --le:#70a8d8; --ls:#60c8c0;
  --success:#52c98a; --warn:#f4a44a;
  --sans:'Inter',system-ui,sans-serif; --mono:'JetBrains Mono',monospace;
  --r1:5px; --r2:8px; --r3:12px;
}
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
html,body{height:100%;font-family:var(--sans);font-size:13px;background:var(--s0);color:var(--t0);overflow:hidden;-webkit-font-smoothing:antialiased}
body{display:flex;flex-direction:column}

/* ── Titlebar ── */
.titlebar{display:flex;align-items:center;gap:8px;padding:0 14px;height:46px;background:var(--s1);border-bottom:1px solid var(--s4);flex-shrink:0}
.tl-logo{display:flex;align-items:center;gap:8px;font-size:13px;font-weight:600;color:var(--t0);letter-spacing:-.01em;margin-right:6px}
.tl-logo i{font-size:18px;background:linear-gradient(135deg,var(--lh),var(--a));-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.tl-logo span{color:var(--a)} .tl-sep{flex:1}
.vsep{width:1px;height:18px;background:var(--s4);margin:0 2px}
.tbtn{font-family:var(--sans);font-size:12px;font-weight:500;padding:5px 11px;border-radius:var(--r1);border:1px solid var(--s4);background:transparent;color:var(--t1);cursor:pointer;display:inline-flex;align-items:center;gap:5px;transition:all .15s;white-space:nowrap}
.tbtn i{font-size:13px}
.tbtn:hover{background:var(--s2);border-color:var(--s5);color:var(--t0)}
.tbtn.hi{background:var(--a-bg);border-color:var(--a-bd);color:var(--a)}
.tbtn.hi:hover{background:rgba(108,138,255,.2)}
...
/* ── Resize handles ── */
.resizer{background:var(--s4);cursor:col-resize;transition:background .15s;z-index:10;touch-action:none}
.resizer:hover,.resizer.active{background:var(--a)}

/* ── Sidebar / Inspector panels ── */
.sidebar,.insp{display:flex;flex-direction:column;min-height:0;overflow:hidden}
.sidebar{background:var(--s1);border-right:1px solid var(--s4)}
.insp{background:var(--s1);border-left:1px solid var(--s4)}
.panel-hd{font-size:10px;font-weight:600;color:var(--t2);letter-spacing:.1em;text-transform:uppercase;padding:10px 12px 8px;border-bottom:1px solid var(--s4);flex-shrink:0}
.pal-list{padding:9px 8px;display:flex;flex-direction:column;gap:4px;overflow-y:auto;flex:1}
.pal-group{font-size:10px;font-weight:600;color:var(--t2);letter-spacing:.08em;text-transform:uppercase;padding:8px 4px 4px}

/* ── Palette cards ── */
.pc{padding:8px 10px;border-radius:var(--r2);border:1px solid var(--s4);cursor:grab;user-select:none;background:var(--s2);transition:border-color .15s,background .15s,transform .1s;position:relative}
.pc:hover{border-color:var(--s5);background:var(--s3);transform:translateY(-1px)}
.pc:active{cursor:grabbing;opacity:.7;transform:translateY(0)}
.pc-name{font-size:12px;font-weight:500;color:var(--t0);display:flex;align-items:center;gap:6px}
.pc-name i{font-size:14px}
.pc-hint{font-size:10px;color:var(--t2);margin-top:2px;padding-left:20px}
...
/* ── Canvas ── */
.canvas-col{display:flex;flex-direction:column;min-height:0;background:var(--s0)}
.canvas{flex:1;overflow-y:auto;padding:12px;display:flex;flex-direction:column;gap:9px;align-items:stretch}

/* ── Lanes ── */
.lane{border-radius:var(--r3);border:1px solid var(--s4);background:var(--s1);flex-shrink:0;transition:border-color .15s}
.lane-hd{display:flex;align-items:center;gap:9px;padding:9px 13px;border-radius:var(--r3) var(--r3) 0 0;cursor:pointer;transition:background .15s;border-bottom:1px solid transparent}
.lane-hd.has-cards{border-bottom-color:var(--s4)}
.lane-hd.over{background:var(--a-bg);outline:1.5px dashed var(--a-bd);outline-offset:-2px;border-radius:var(--r3) var(--r3) 0 0}
.lane-hd.reject{background:var(--d-bg);outline:1.5px dashed var(--d);outline-offset:-2px}
...
.lane-badge{font-size:10px;font-weight:500;color:var(--t2);font-family:var(--mono)}
.lane-add-hint{font-size:10px;color:var(--t2);font-style:italic}
.lane-body{padding:8px;display:flex;flex-direction:column;gap:6px;min-height:0;border-radius:0 0 var(--r3) var(--r3)}
/* body drop zone (secondary fallback) */
.lane-body.over{background:var(--a-bg);outline:1.5px dashed var(--a-bd);outline-offset:-2px;min-height:44px}
.lane-body.reject{background:var(--d-bg);outline:1.5px dashed var(--d);outline-offset:-2px;min-height:44px}
.lane-empty{font-size:11px;color:var(--t2);padding:6px 4px;text-align:center;font-style:italic}

/* ── Cards ── */
.card{border-radius:var(--r2);border:1px solid var(--s4);background:var(--s2);cursor:pointer;transition:border-color .15s,background .15s;user-select:none}
.card:hover{border-color:var(--s5);background:var(--s3)}
.card.sel{border-color:var(--a);background:var(--s2);box-shadow:0 0 0 2px var(--a-bg)}
...
.card-rm{font-size:12px;color:var(--t2);opacity:.4;cursor:pointer;padding:2px 4px;border-radius:4px;line-height:1;flex-shrink:0;transition:opacity .12s,color .12s}
.card-rm:hover{opacity:1;color:var(--d)}
.card-body{padding:1px 10px 8px;display:flex;flex-direction:column;gap:3px}
.cr{font-size:10px;color:var(--t1);display:flex;gap:5px;align-items:baseline;flex-wrap:wrap}
...
/* ── Inspector ── */
.insp-body{padding:11px;overflow-y:auto;flex:1;display:flex;flex-direction:column;gap:10px}
.insp-nil{display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:7px;opacity:.3}
.insp-nil i{font-size:26px} .insp-nil p{font-size:11px;text-align:center;line-height:1.5}
.isec{font-size:10px;font-weight:600;color:var(--t2);letter-spacing:.08em;text-transform:uppercase;padding-bottom:5px;border-bottom:1px solid var(--s4);margin-bottom:1px}
.fg{display:flex;flex-direction:column;gap:4px}
.fl{font-size:11px;font-weight:500;color:var(--t1)}
.fhint{font-size:10px;color:var(--t2);margin-top:-1px}
.fi{font-family:var(--sans);font-size:12px;padding:6px 9px;border-radius:var(--r1);border:1px solid var(--s4);background:var(--s2);color:var(--t0);width:100%;outline:none;transition:border-color .15s}
.fi:focus{border-color:var(--a)} .fi:disabled{opacity:.3;cursor:not-allowed}
...
.cn-preview{font-family:var(--mono);font-size:11px;padding:5px 9px;border-radius:var(--r1);background:var(--s0);border:1px solid var(--s4);color:var(--t0);display:flex;align-items:center;margin-top:2px}
.cn-fixed{color:var(--t2)} .cn-core{color:var(--a);font-weight:500}
...
.lane-err-badge { font-size:10px; font-weight:600; color:var(--d);
...
.modal{background:var(--s1);border-radius:var(--r3);border:1px solid var(--s5);width:560px;max-height:590px;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 24px 64px rgba(0,0,0,.6)}
.modal-hd{display:flex;align-items:center;justify-content:space-between;padding:13px 16px;border-bottom:1px solid var(--s4);flex-shrink:0}
.modal-hd h3{font-size:13px;font-weight:600;color:var(--t0)}
.modal-cl{font-size:17px;cursor:pointer;color:var(--t1);padding:2px 5px;border-radius:4px;transition:color .12s}
.modal-cl:hover{color:var(--t0)}
.modal-body{padding:15px;overflow-y:auto;flex:1}
.msg-pre{font-family:var(--mono);font-size:11px;line-height:1.9;color:var(--t0);white-space:pre-wrap;word-break:break-all;background:var(--s0);padding:12px 14px;border-radius:var(--r2);border:1px solid var(--s4)}
...
/* ── Intro overlay ── */
.intro-overlay{position:fixed;inset:0;z-index:200;display:flex;align-items:center;justify-content:center;background:rgba(10,12,20,.92);backdrop-filter:blur(6px)}
.intro-box{background:var(--s1);border:1px solid var(--s5);border-radius:var(--r3);width:600px;max-height:90vh;overflow-y:auto;box-shadow:0 32px 80px rgba(0,0,0,.7);display:flex;flex-direction:column}
.intro-hero{padding:32px 36px 24px;border-bottom:1px solid var(--s4);background:linear-gradient(135deg,#12172a,#1a2040)}
.intro-hero-logo{font-size:22px;font-weight:700;color:var(--t0);letter-spacing:-.02em;margin-bottom:4px}
.intro-hero-logo span{background:linear-gradient(90deg,var(--lh),var(--a));-webkit-background-clip:text;-webkit-text-fill-color:transparent}
...
.intro-section-title{font-size:11px;font-weight:600;color:var(--t2);letter-spacing:.08em;text-transform:uppercase;margin-bottom:8px}
.intro-steps{display:flex;flex-direction:column;gap:10px}
.intro-step{display:flex;gap:12px;align-items:flex-start}
.intro-step-num{width:22px;height:22px;border-radius:50%;background:var(--a-bg);border:1px solid var(--a-bd);color:var(--a);font-size:11px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:1px}
...
.intro-copy{font-size:11px;color:var(--t2)}
...
.pal-preset-grid { display:grid; grid-template-columns:1fr 1fr; gap:4px; padding:0 0 4px; }
.pal-preset {
  font-size:10px; font-weight:500; padding:5px 6px;
  border-radius:var(--r1); border:1px solid var(--s4);
  background:var(--s2); color:var(--t1);
  cursor:pointer; text-align:left;
  display:flex; align-items:center; gap:5px;
  transition:all .15s; white-space:nowrap; overflow:hidden;
}
.pal-preset i { font-size:12px; flex-shrink:0; }
.pal-preset:hover { border-color:var(--s5); background:var(--s3); color:var(--t0); }


.footer-panel {
  background: var(--s1);          /* dark panel */
  color: var(--t0);                  /* soft text */
  padding: 10px 30px;           /* breathing room */
  border-top:2px solid var(--s4);   /* subtle separation */
